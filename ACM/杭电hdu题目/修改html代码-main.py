...
def change_t2(tt2):
    ...
    try:
        ...
        t6 = '.狟婥" type="text/javascript">'
        tt5 = tt4.split(t6)
        tt6 = t6.split(".狟婥")  # 编码问题
        t6 = tt6[1]
        t5 = '"></script><script src="' + 'HTML-Files/' + tt5[0]  # "></script><script src="HTML-Files/MathJax.js
        t7 = tt5[1]
        tt7 = t7.split('href="')
...
def change_file(scene_name, file_name):
    name_html = "HTMLs/"
    # with open(name_html+file_name, 'r', encoding='utf-8')as f:
    f = codecs.open(name_html+file_name, encoding="big5")
    a = f.read()
    f.close()
        # dbg(a)
    ...
    with open(file_name_2, 'w', encoding="gbk")as ff:
        ff.write(t)
        ff.close()
