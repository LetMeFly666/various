const begin = ''; const end=''; const problem_description_data = '\<div class="subject-question"><div>  24dian is a famous game. You are given a set of n cards, each card contains an integer between 1 to 13. You can put the cards in arbitrary order, add + or - or * or / between any two cards, and add as many parentheses as you want. Your goal is to make the expression valid and its value equals m. <br/> However, there\re cases when valid solutions exist, but all the valid solutions involve fractions, that is, non-integers, in the process of calculation. For example, if you want to use (1, 5, 5, 5) to get 24, the only solution is 5*(1-(1/5)), where 1/5 is a fraction. <br/> Given the number of cards n and the result m you want to get, find all the possible set of cards that has a valid solution but any solution involves fraction in the calculation. <br/> </div> <div> </div></div>\n'; const input_description_data = '\<pre>The first line contains two integers <img src="https://www.nowcoder.com/equation?tex=n%2Cm(1%5Cleq%20n%5Cleq%204%2C1%5Cleq%20m%5Cleq%2010%5E9)"/> , representing the number of cards and the result you want to get. </pre>\n'; const output_description_data = '\<pre><div>First line contains an integer k, denoting the number of possible sets. </div><div>In next k lines, each line contains n non-decreasing numbers, represent a possible set. You should print the sets in lexicographical order.</div></pre>\n';
